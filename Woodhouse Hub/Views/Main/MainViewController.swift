...
class MainViewController: UIViewController {
	...
	internal func showDisclaimerMessage() {
		let message = "\nThis application has been made with the intention of aggregating Woodhouse College's Learning Management and student facing systems. No harm is intended to the developers of the original systems. IMO the current systems are slow, dated and unintuitive to use. This application is a mitigation to those factors, by taking advantage of local storage to store student's details and their timetable as examples, as well as extending the system to notify each student for lessons. This application is in compliance with GDPR, since it piggybacks off of existing systems. Students can only access their information with their login details, unique to them. I am now an alumni of the college and have no affiliation directly with the development teams responsible for Dashboard, Woodle, ReportServer or any other digital services used by the college."
